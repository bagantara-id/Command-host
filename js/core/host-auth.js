...
export class HostAuthenticator {
    constructor() {
        ...
        // Kunci mutlak: Tunggu HTML siap sebelum mengeksekusi auth
    ...
    monitorAuthState() {
        onAuthStateChanged(auth, (user) => {
            if (user) this.verifyClearance(user);
            else this.lockdownSystem();
        });
    }
    ...
    verifyClearance(user) {
        if (user.email === AUTHORIZED_EMAIL || user.uid === AUTHORIZED_UID) {
            ...
            if (userDisplay) userDisplay.innerText = `AUTHENTICATED: ${user.email}`;
        } else {
