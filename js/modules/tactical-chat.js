...
import { collection, query, orderBy, onSnapshot, addDoc, serverTimestamp, doc, deleteDoc, setDoc, getDocs, writeBatch } from "https://www.gstatic.com/firebasejs/10.8.1/firebase-firestore.js";
...
export class TacticalChat {
    ...
    async executeKillSwitch() {
        ...
        try {
            // 1. Hapus seluruh riwayat pesan (sub-koleksi messages)
            ...
            // 2. Hapus seluruh data log telemetri (sub-koleksi telemetry)
            ...
            // 3. Hapus dokumen utama sesi di Firestore
            await networkTacticalTimeout(deleteDoc(doc(db, "sessions", roomId)), 5000);

            // 4. Hapus access_keys (Sinyal pemusnahan terakhir untuk memicu self-destruct di klien)
            await networkTacticalTimeout(deleteDoc(doc(db, "access_keys", roomId)), 5000);

        } catch (e) {
            console.error("[KILL-SWITCH] Eksekusi darurat fallback:", e);
            // Fallback: hapus kunci akses langsung jika batch terhambat
