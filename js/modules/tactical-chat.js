...
export class TacticalChat {
    ...
    startRadarTelemetry() {
        this.unsubscribeTelemetry = onSnapshot(doc(db, `sessions/${this.currentRoomId}/telemetry/data`), (docSnap) => {
            ...
            const btnCopyGeo = document.getElementById('btnCopyGeo'); // Ekstensi HTML
            if(!display) return;
    ...
    animateRadar() {
        ...
        const draw = () => {
            if (!this.currentRoomId) return; // Hentikan render jika sesi mati
    ...
    async executeKillSwitch() {
        ...
        try {
            await networkTacticalTimeout(deleteDoc(keyRef), 8000); 
        } catch(e) {}
        this.terminate();
