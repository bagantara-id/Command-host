...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (window.location.hostname === 'localhost' || window.location.hostname === '127.0.0.1') {
        ...
        if(guestDomainInput) guestDomainInput.value = 'https://domain-anda.github.io/sesi';
    }
    ...
    const syncDRMState = async () => {
        if (!tacticalChat.currentRoomId) return; // Abaikan jika tidak ada sesi aktif
    ...
    // PERBAIKAN: Pemusatan Logika Input Teks & Tombol Kirim
    ...
    // PERBAIKAN: Pemusatan Logika Kill-Switch Tanpa Kloning Node
    if (btnKillSwitch) {
