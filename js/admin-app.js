...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Fitur Baru: Salin & Sapu Memori
    const btnCopyUrl = document.getElementById('btnCopyUrl'); // Tambahkan tombol ini di HTML Admin
    const btnWipeMemory = document.getElementById('btnWipeMemory'); // Tambahkan tombol ini di HTML Admin

    // KOREKSI MUTLAK: Path Repositori
    ...
    // UTILITAS COPY-TO-CLIPBOARD TACTICAL
    const copyTacticalData = async (text, btnElement) => {
        if (!text || text.includes("MENUNGGU") || text.includes("GAGAL")) return;
        try {
            await navigator.clipboard.writeText(text);
            const originalText = btnElement.innerText;
            btnElement.innerText = "☑ DISALIN";
            btnElement.style.background = "var(--quantum-cyan)";
            ...
            console.warn("Akses Clipboard diblokir sistem.");
        }
    };

    if (btnCopyUrl && urlInput) {
        btnCopyUrl.addEventListener('click', () => copyTacticalData(urlInput.value, btnCopyUrl));
    }

    // PROTOKOL SANITASI MEMORI (ZERO-DISRUPTION WIPE)
    if (btnWipeMemory) {
        btnWipeMemory.addEventListener('click', async () => {
            if (!confirm("Sanitasi memori akan membersihkan Cache dan RAM tanpa memutus sesi klien. Lanjutkan?")) return;
            ...
            [localStorage, sessionStorage].forEach(storage => {
                Object.keys(storage).forEach(key => {
                    if (!key.startsWith('firebase:')) storage.removeItem(key);
                });
            });

            if (tacticalChat.currentRoomId) {
                const activeRoom = tacticalChat.currentRoomId;
                tacticalChat.terminate();
                setTimeout(() => tacticalChat.initialize(activeRoom), 150);
            }
        });
    }

    // FUNGSI SARAF REAL-TIME DENGAN TIMEOUT (ANTI-FREEZE)
    const syncDRMState = async () => {
        ...
        try {
            await networkTacticalTimeout(updateDoc(sessionRef, {
            ...
            console.log("[GOD'S EYE] Saraf DRM berhasil diperbarui.");
        } catch (e) {
            console.error("[ANOMALI] Jaringan terhambat:", e);
        }
    };
    ...
    if (btnGenerateKey) {
        btnGenerateKey.addEventListener('click', async () => {
            ...
            try {
                ...
                urlInput.value = `GAGAL: Koneksi diblokir / memburuk.`;
